...
export const getAllAchievements = async (req, res) => {
  ...
  const achievements = await Achievements.find(queryObject)
    .sort(sortKey)
    .skip(skip)
    .limit(limit)

  const totalAchievements = await Achievements.countDocuments(queryObject)
